...
import image from './excitment.png'
...
const getClientName = (clientId) => {
  ...
  console.log(ProjectsFilter, "filtered with waiting status")

  console.log(selectedConsultant)
 console.log(selectedEmployee,"dddd")
  ...
  console.log(filteredProjects,"still checking")
  const [monthsPassed, setMonthsPassed] = useState("");

  const handleInputChange = (e) => {
    setMonthsPassed(e.target.value);
  };

  const filterProjectsByDeadline = () => {
    const today = new Date();
    return filteredProjects.filter((project) => {
      const deadlineDate = new Date(project.deadline);
      const monthsDifference =
        (today.getFullYear() - deadlineDate.getFullYear()) * 12 +
        today.getMonth() -
        deadlineDate.getMonth();
      return monthsDifference >= monthsPassed;
    });
  };

  const newFilteredProjects = filterProjectsByDeadline();

  const totalFinancialProposal = newFilteredProjects.reduce((total, project) => total + project.financial_proposal, 0);
  return (
...
<div className='pl-2'>
<label className="mr-2 font-bold">Enter months after submission:</label>
<input
        type="number"
        placeholder="Enter months since deadline passed"
        value={monthsPassed}
...
<Select
  ...
  />
            ...
            <div className="overflow-y-auto overflow-x-auto h-5/6" >
          <table className="min-w-full bg-white border border-gray-200">
            ...
            <tbody>
              {newFilteredProjects.map((project) => (
                ...
                >
                  ...
                  <td
                  onClick={()=>handleProjectModalClick(project)}
                  className="px-3 py-2 max-w-18">{project.type_of_bid}</td>
                  <td
                  onClick={()=>handleProjectModalClick(project)}
      ...
      <motion.img 
        src={image} 
        alt="Excitement sticker" 
        className="w-24 h-24 ml-4"
        animate={{ y: [0, -20, 0] }} // Up and down movement
        transition={{ duration: 0.8, repeat: Infinity, repeatType: 'mirror' }} // Looping the bounce effect
      />
    </div>
              <p className=' pb-8' >You are going to confirm that we won "{selectedProject?.name}"?</p>
   
              <div className='pl-2'>
<label className="mr-2 font-bold">Enter the value of the contract:</label>
<input
        type="number"
      ...
      />
        ...
        {showNoModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-75">
            <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
              <h2 className="text-2xl font-bold mb-4">Reason of Failing</h2>
              <p>Why did we fail the project "{selectedProject?.name}"?</p>
              <textarea
